...
module.exports = {
    ...
    findMessages : function(id2Topic,suunt, callback) {
        ...
        con.connect(err => {
            ...
            con.query("SELECT a.* FROM messages a WHERE a.topic LIKE (SELECT b.topic FROM messages b WHERE ?=b.id ) ORDER BY a.id "+suunta,[id2Topic], (err, data) => {
                if(err) return callback(null);
                con.close();
    ...
    findUserMsgs : function(userid,suunt, callback) {
        ...
        con.connect(err => {
            ...
            con.query("SELECT * FROM messages WHERE sender=? ORDER BY id "+suunta, [userid], (err, data) => {
                if(err) return callback(null);
                con.close();
                return callback(data);
            });
    ...
    */
    ...
    findSingleMsg : function(iidee,userid, callback) {
        ...
        con.connect(err => {
            ...
            con.query("SELECT * FROM messages WHERE id=? AND sender=?",[iidee,userid], (err, data) => {
                if(err) return callback(null);
                con.close();
                return callback(data);
            });
    ...
    editMessage : function (req, callback ) {
        ...
        con.connect(err => {
            if (err) {
                ...
                con.query("UPDATE messages SET message=? WHERE id=? AND SENDER=?",[req.message,req.muokkaa,req.sender], (err, result) => {
                    if (err) return callback(false);
                    con.query("COMMIT", (err, ans) => {
                                                       con.query("SELECT * FROM messages", (err, data) => {
                                                           if (err) return callback(false);
    ...
    deleteMessage : function (delid, userid, callback ) {
        ...
        con.connect(err => {
            if (err) return callback(null);
            // Poimitaan otsikot, viestien määrä sekä uusimman viestin id ja lähettäjä
            con.query("DELETE FROM messages WHERE id=? AND SENDER=?",[delid,userid], (err, data) => {
                if(err) return callback(null);
                con.query("COMMIT", (err, ans) => {
                                                   con.query("SELECT * FROM messages", (err, data) => {
                                                       if (err) return callback(false);
